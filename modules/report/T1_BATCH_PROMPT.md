...
## 진행 상황 (2026-07-14 세션 — 2본 완주)

- ✅ **고려아연 010130 (에너지소재/제련) 완주** — 커밋 `14c0a42`. 5게이트 PASS. V-056~059. 첫 T1 대표 = **재현 레시피 원형**.
- ✅ **현대건설 000720 (건설) 완주** — 커밋 `030c0c8`. 5게이트 PASS(정확성 REFUTED 0·완전성 eq-div 추가로 shallow 해소·렌더 실측). V-060·061. tier `1c→1`·galaxy_index 8본. 건설 진행기준·미청구공사·PF보증. **V-056 운전자본형 −OCF 2회째(제련+건설)** → assemble 규칙 승격 후보.
- ⚠️ **회사별 함정(V-061, 다음 3본 착수 전 필독)**: ① **account_id 시점 변경**(sgna·div 등이 연도별 다른 계정코드) → series.py 단일 account로 결측 → 빌드에서 후보 account **per-year 병합**해 골든 series 주입(check_golden은 골든 series 읽음). ② **fact 단위 혼재**(주석마다 천원/백만원 다름) → `fact_mn`이 `unit` 필드 읽어 환산(고정 나눗셈 금지). ③ **tci 파생 이중반올림** → 실제 CIS ComprehensiveIncome로 정정. ④ capex/div 진짜 결측(3yr)이면 ppe/eq-div dive 생략(R6.9), 그러나 account_id 병합으로 완결 가능하면 dive 추가(완전성 감사가 eq-div 누락을 shallow로 잡음).
...
- ⏳ **남은 3본: SKT 017670 · HMM 011200 · KT&G 033780** (주석 완비 확인) + 한화 012450 보류. facts_000720.json은 생성됨(현대건설, 로컬).
