...
DART 표는 병합 셀(colspan/rowspan)이 많아 셀을 좌→우로 단순 평탄화하면 **열이 어긋난다**(자본변동표 등). `_table_grid()`가 병합을 **직사각 격자로 전개**한다 — (r,c) 셀맵에 배치하되 윗줄 rowspan이 점유한 칸은 건너뛰고, 병합 확장칸은 `''`로 채워 정렬 보존. 완전 빈 열은 제거. 결과: 모든 행이 동일 열 수 → 렌더가 정렬됨.
초장문 표(종속기업/계열사 목록 등)는 `_ROW_CAP`(기본 60)행까지만 + "총 N행" 표시 — 파일 비대 방지(삼성 note1 491행 → 3MB가 주범이었음).
