...
| 8 | 015760 | 한국전력 | 유틸리티 | 규제산업·대규모 차입·적자↔흑자 반전. ⚠️ **S0에서 주16 괴물블록(>20만자) 기보고 — 재섹션·별도FS 유입 확인 필수** |
...
**데이터 상태(2026-08-03 실측)**: 8본 전부 FY2021~25 **5개년 원문 + 최신연도 주석 섹셔닝 완비**(29~47노트).
`fs_account`는 **한국전력만 완비**, 나머지 7본은 `fs_enrich` 필요(Phase 2에서 일괄). 착수 전 `series` 완결률 확인.
