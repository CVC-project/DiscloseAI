...
**행 라벨 = 재무제표 원문 계정명(R6.6a)** — 패널 행 `name`은 **원문 계정명 그대로**(자본금·이익잉여금·비지배지분·사채…). 쉬운 설명은 **딥다이브 카드가** 담당하고, 행에는 원문명 뒤 **괄호 보완**(금칙어 아닌 짧은 부연, 예 `현금및현금성자산 (위 강조 행과 같은 돈)`)만 허용 — 원문명을 **대체**하는 의역(`밑천`·`그동안 쌓은 이익`·`거의 0`·`자회사의 다른 주주 몫`)은 금지.
