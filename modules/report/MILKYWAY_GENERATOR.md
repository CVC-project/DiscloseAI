...
- **S0(착수 전 읽기)**: 새 티커 착수 전 **VARIATIONS.md(정본) 전체**를 읽는다 — 코드화 색인·구조 패턴표·활성 채록·로그로 재편돼(2026-08-04, 리더 지시) 한 번에 읽힌다. 패턴표가 가리키는 과거 번호의 전문은 [VARIATIONS_ARCHIVE.md](VARIATIONS_ARCHIVE.md)(V-001~100, 읽기 전용)에서 **필요한 번호만** 조회한다.
- **S7(완주 후 쓰기)**: 완주마다 ① 새 편차를 V-### 항목으로 **정본 §4(활성 채록)에** 채록(증상→원인→**왜 게이트가 못 잡았나**→처리→적용 범위 — V-106 교훈: 사각 명시까지가 채록이다) ② 채록 로그 표에 회사 1줄 추가 — **신규 변형이 없어도 '신규 변형 없음'을 명시**. 이게 없으면 완주가 아니다. 코드 승격이 끝나 본문이 더 안 읽히는 항목은 리더 승인 하에 아카이브로 이동(번호 개서 금지).
...
## 부록 A. 확장 로드맵 — 실측 기준 (2026-08-12 갱신)

> 근거: `fs_parse` 전 상장사 완주로 **2,590사 재무 데이터 확보**([FS_PARSE_PLAN.md](FS_PARSE_PLAN.md) §10).
> 병목이 뒤집혔다 — **데이터가 아니라 해석 템플릿(골든)이 부족**하다.

### A.1 축이 둘인데 하나만 다뤘다

골든 20본·lite 23본이 **전부 KOSPI**다. 그런데 데이터가 확보된 2,590사 중 **KOSDAQ이 1,761사(68%)**이고,
두 시장은 **재무 프로필이 다른 세계**다 (FY2024, 연결·당기 실측):

| | KOSPI | KOSDAQ |
|---|---|---|
| 파싱 완료 | 827 | **1,761** |
| 골든/lite | 20 / 23 | **0 / 0** |
| 골든 산업 커버 | 182 (22.0%) | 297 (16.9%) — **미커버 1,464** |
| 매출 중앙값 | 6,352억 | **699억** (9배) |
| 영업적자 비율 | 20.3% | **39.7%** |
| 매출 1,000억 미만 | 13.1% | **60.1%** |

→ 기존 골든의 서술 틀(대규모 매출→원가→이익 · 대규모 capex · 부문정보)은 매출 700억·적자 40%인
집단에 그대로 못 쓴다. **산업축(19개 완료) × 규모·수익구조축(대형 KOSPI만)** 중 후자가 통째로 비어 있다.

### A.2 신규 골든 우선순위

| 순위 | KSIC 소분류 | 기업 | 산업 | 근거 |
|---|---|---|---|---|
| **1** | **292** | **163** (KOSDAQ 145) | 특수 목적용 기계 (반도체·디스플레이 장비) | 갭 최대 + 기존 반도체 골든의 **하류 밸류체인** → 서술 재사용 최대. 이 1본이 "KOSDAQ 소형 제조" 틀을 세운다 |
| 2 | 582(5822*) | 143 | 소프트웨어 (SI·솔루션·보안) | 게임 골든(크래프톤 5821)이 안 맞는 집합. 용역매출·개발비 |
| 3 | 303 | 103 (KOSDAQ 56·KOSPI 47) | 자동차 신품 부품 | **양 시장 공통**. 현대차 골든의 하류 |
| 4 | 262 / 212 / 204 | 132 / 104 / 94 | 전자부품 · 의약품 · 기타화학 | 양 시장 공통, 산업축만으로 해결 |
| 별도 유형 | 701 + 적자형 | 62 + 229사 | 연구개발업(바이오텍) | 매출 미미·적자가 **정상**. 기존 서술 틀 자체가 안 맞아 **새 유형 설계** 필요 → 뒤로 |
| 별도 트랙 | 금융 | 118 | — | top line 정의부터 다름(P4에서도 제외) |

상위 3본이면 **+409사**, 커버 479 → 약 888사. 상위 6본이면 **+739사**(약 1,218사, 47%).

> ⚠️ 커버율은 **산업 코드 기준**이다. 골든 1본이 그 산업 전체에 실제로 쓸 만한지는 lite 생성에서 드러난다.
> 292는 장비사끼리도 전공정/후공정/디스플레이로 갈려 1본으로 145사가 다 덮이지 않을 수 있다.

### A.3 선결 — 신규 대상은 `fs_account`가 비어 있다

`series.py`는 **경계 단방향 원칙상 `fs_account`(reports.db)만 읽는다** — firm_json은 integration 소유라 안 읽는다.
그런데 `fs_enrich`(DART API)는 골든 클러스터 **55사에만** 돌렸다. 실측(원익IPS 240810):

```
report_raw 5개년 ✓ · report_section 182건 ✓ · fs_account_xml 384행 ✓ · firm_json ✓
fs_account 0행  →  series 완결 0/24
```

→ 신규 골든 착수 전 **`fs_enrich <ticker>` 실행이 필수**(DART 키). 갈림길은 [NEXT_SESSION.md](NEXT_SESSION.md) 참조.
