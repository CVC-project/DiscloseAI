...
| 5.5 | **U-확대** | ✅ 완료(2026-07-29) — U1~U3 "완료"의 실제 잔여 해소. ① **dart_filing 소실 사고 수리**(filters prune이 생산자 소유 행을 매 실행 오인 삭제 — 115엣지가 DB에 0행이었음, 회귀 박제) ② **T1 주석 경로 링킹 방어층**(L1·L2 공용화 — FN-013 재발 구조 차단) ③ **제목망 접두 규칙**(노트 1,356→1,550) + **계층헤더 HTML 그리드 파서 신설**(미파싱 856→379) ④ **DART 지분 2025 증분**(56,781행·에러0). 커버리지: valuechain 256→**771**·rp_note 주체 23→**373사**·dart_filing 0→**136사** | 〃 | ✗ | pytest 663 passed · V-3 53/53(fail 0) · 신규 커버 3사 60/60 · G-C 30건 CPA 대기 |
| 5.7 | **U5 비상장·개인 노드** | ✅ 완료(2026-07-29, 리더 B안) — **비상장은 앵커-로컬 별개 노드, 공시 원문 문자열 그대로, 노드 간 연관성 탐색 없음**(전역 병합·표기 통합 안 함 → 동명 병합·공제조합 허브 오염 리스크 **설계상 소멸**). `UnlistedNode` + `entity_kind` 분류기(kind 4종) → filters·related_party 양 경로 적재(prune은 참조 무결성 기준) → export `t` 유무 판별자·상한 60·companies_index 불변 → EgoView 무채+형태+알파 강등·NODE TYPOLOGY 범례·클릭=팝오버(re-root 없음, [UX-024](../../../integration/v2/UX_DECISIONS.md)). **관계 보유 1,483→2,614 / 2,651**(관계 0은 37사). 상세 PROGRESS 후속 13 | `feat/relation-universe-viz` | ✗ | ✅ 관계0 1,202→**37**(≤200) · V-3 53/53 + S4/S5 32/32 · G-C 30건 CPA 대기 |
...
|   ↳ 진행상황(2026-07-22) | `related_party.parse_governance_categories()`/`apply_governance()` 구현 완료 — "구분/특수관계자명" 2-컬럼 카테고리 표(삼성바이오로직스·SK이노베이션·HD현대 계열 확인, 최소 5개사) → RelationLocal(dart_filing) 109노트→43엣지. 나머지 표 변형(와이드 1행형·행=개별회사형)은 미구현 | **게이트 PASS (2026-07-22)** — CPA(리더) 원문 대조 검수. graph/build.py 연도별 dedupe 수정과 같은 논리로 "최신 연도만 유효 상태"를 적용하면 dart_filing 실제 모집단은 43건이 아니라 **22건(2개사만 — SK이노베이션 9·삼성바이오로직스 13, HD현대 계열 3사는 이 표 형태에서 0건 생존)**. 이 22건 전수 대상 CPA 검수 결과 확인된 20/22 전부 정확(SK텔레콤·SK디스커버리·SK가스·SK디앤디·SK케미칼·SK하이닉스·SK네트웍스·SKC·SK(지배기업) 및 삼성바이오로직스 지배기업/유의적영향력/관계기업/대규모기업집단 목록 다수) — 미확인 2건(유일로보틱스·에임드바이오)이 최악의 경우 둘 다 틀려도 20/22=0.909 ≥ 0.85 임계 자동 충족. **G-C 통과 확정.** | 〃 |
|   ↳ 추가 구현(2026-07-22, 같은 날 후속 세션) | 나머지 표 변형 5가지 중 4가지 추가 구현(라벨 변형·와이드 1행형·행=개별회사형·거래표-임베디드형·캐리포워드형) → dart_filing 43→**115엣지**, 소스 회사 2개사→**7개사**(신규: 카카오·고려아연·현대로템·삼성전자·HD한국조선해양·삼성물산·한화에어로스페이스·엘에스일렉트릭·우리금융지주·두산). LIG디펜스앤에어로스페이스는 원본 소스 자체 구분자 유실로 영구 미구현. | **위 G-C PASS 판정은 원 22건(2개사) 범위만 CPA 검수됨 — 이번에 추가된 4가지 파서·93건(5개사분)은 각각 실제 공시 fixture 기반 단위테스트(카테고리·회사명 정확 대조)로만 검증, 별도 CPA 스팟체크는 아직 안 거침.** 코드 구현 신뢰도는 각 파서마다 원문 HTML(ROWSPAN/COLSPAN 보존) 직접 열람 후 구현했지만, 게이트를 신규분까지 확장 적용하려면 추가 스팟체크 권장(선택, 급하지 않음 — 리더 판단). | 〃 |
