...
- ❌ T2 서술 추출 지배구조 엣지 — U4 이연. **GPU 서버 복구가 선행 조건** (2026-07-21 현재 unreachable)
...
| **GPU/A100** | 없음 — U0~U3은 전부 CPU·API 경로 | U4(T2 추출)에서만 등장. 서버 복구 전 U4 착수 금지 |
...
| # | 결정 | 근거 |
|---|---|---|
| U-D1 | 정형 지분 엣지 1차 원천 = **DART API `hyslrSttus` + `otrCprInvstmntSttus` 전 상장사 확장** — 기존 `ingest/dart.py` 호출부는 corp_code 파라미터 범용이므로 루프 대상만 top50 → CompanyRegistry로 교체. 호출량: 최신 연도 ~5,200건(2,600×2) = 하루 완주, **5개년 백필 ~2.6만 건 = 일 1만 한도 내 3일 배치** | 정형·결정적·기수집 파이프라인 재사용. 원문 파싱보다 싸고 빠름 |
...
| U-D4 | ftc_group 엣지는 **스타 토폴로지**(동일인·대표회사 → 소속 상장사)로 저장 — 집단 내 클리크(O(n²)) 금지. "같은 집단" 강조는 노드 `group` 속성으로 표현 | 삼성 계열 상장사 ~17곳 클리크 = 136엣지 폭발 방지 |
...
| U-D11 | Universe·섹터 레벨의 엣지 렌더는 **활성·hover 노드의 엣지만**(현행 SectorMap 동작을 그대로 스케일) — 상시 전체 엣지 도포 금지. 섹터 간 집계 흐름선은 U4(valuechain V4 합류) | 렌더 예산 + 헤어볼 금지 |
...
| 엣지 예산 | 활성/hover 노드의 엣지만 (U-D11). 상시 전체 엣지 금지 |
...
| 팔레트 확장 | 신규 섹터 = integration이 `SECTOR_DEF`(adapter.js)와 `SECTOR_PALETTE`(bundle.jsx) **동시 추가** — ⚠️ adapter의 buildPalette가 SECTOR_DEF 미등록 섹터를 필터로 탈락시켜 **섹터가 조용히 사라짐** (U2 QA 항목) |
...
|---|---|---|---|
| **U0 기반** | valuechain V0와 **동시 실행**: CompanyRegistry에 universe 컬럼 포함해 1회 마이그레이션, registry 2,600 적재(KSIC 포함), 시총 스냅샷, `ksic_sector_map.csv`, top-400 선정, `sectors.json` handoff, pykrx requirements 등록 | registry ≥ 2,500 · 섹터 미매핑 0 · KOSPI 측 상위 200이 KOSPI200 지수 구성종목과 교집합 ≥ 90% (KOSDAQ 측은 시총 순위 자체 검증 — 대응 지수 없음) | `feat/relation-universe-v0` (V0와 한 브랜치 — 스키마 1회 마이그레이션) |
...
| GPU 서버 접속 불가 지속 | U0~U3은 무영향(CPU·API 경로). U4·valuechain V2+가 막힘 — 서버 복구를 독립 선행 태스크로 관리 |
