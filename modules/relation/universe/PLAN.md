...
| 5.7 | **U5 비상장·개인 노드** | 📋 계획 수립 완료(2026-07-29, 리더 B안 결정) — 노드 2,651→~27,700(비상장법인·개인·조합펀드·공공기관 4종 범례, 무채+형태 구분·알파 강등). 실행 정본 [UNLISTED_PLAN.md](UNLISTED_PLAN.md) — U5-a(모델·분류기)→b(파이프라인)→c(export)→d(화면, 목업 게이트)→e(G-C·QA). 착수 전 열린 결정 5건(개인 앵커-로컬·ego-로컬 인덱스 등) 리더 확인 | `feat/relation-universe-viz` | ✗ | 관계0 상장사 1,202→≤200 · G-C ≥0.85 · V-3 확장(S4 강원랜드형) |
