...
| 5.7 | **U5 비상장·개인 노드** | 📋 계획 확정(2026-07-29, 리더 B안 + 열린결정 5건 전부 승인) — **비상장은 앵커-로컬 별개 노드, 공시 원문 문자열 그대로, 노드 간 연관성 탐색 없음**(전역 병합·표기 통합 안 함 → 동명 병합·허브 오염 리스크 설계상 소멸). kind 4종 NODE TYPOLOGY 범례·무채+형태·알파 강등. companies_index 불변(ego-로컬 자급 → 클릭 이동 대상 자연 배제). 실행 정본 [UNLISTED_PLAN.md](UNLISTED_PLAN.md)(**U5-e 완료 후 삭제 — 리더 지시**) — a(모델·분류기)→b(파이프라인)→c(export)→d(화면, 목업 게이트)→e(G-C·QA·문서정리) | `feat/relation-universe-viz` | ✗ | 관계0 상장사 1,202→≤200 · G-C ≥0.85 · V-3 확장(S4 강원랜드형) |
