...
2. **법선 벡터 방향 상쇄 버그 수정 (`draw()`)**:
   - `A→B` 엣지와 `B→A` 엣지의 기본 벡터 방향이 정반대이므로, 오프셋을 줘도 벡터 곱 과정에서 부호가 상쇄되어 **정확히 제자리에 겹쳐버리는 수학적 오류(한화오션·한화시스템 사례)**가 있었음. `if(e.a.n > e.b.n) { px = -px; py = -py; }` 구문으로 법선 벡터 방향을 강제 통일하여 일관된 오프셋 밀어내기를 구현함.
3. **간격 여백 제거 (`init()`)**:
   - `PAD = 0.0`으로 설정. Hover 대비용 `*1.5` 배수를 걷어내고 원래 선의 `width` 값만을 그대로 누적 더함. 서로 여백 단위가 0이 되어, 화면에서 두 색이 물리적으로 딱 맞닿는 시각 처리 완성.
