...
- **★ report 모듈에 보고할 사항(제 담당 밖, read-only라 직접 수정 불가)**:
  ...
  우연이라기엔 너무 정확해 저장 파이프라인의 하드 리밋으로 추정). 하나금융
  지주(대형 금융지주, 표 많음)의 특수관계자 노트가 이 트렁케이션에 걸려
  실제 거래금액 표 내용이 잘려나간 것을 확인(끝부분이 문장 중간에 끊김,
  "매출"/"매입" 문자열 자체가 사라짐). text_md는 영향 없어 보이나(별도 파이프
  라인일 가능성), text_html 의존 파싱(이번 세션 U3·T1 다수)에는 이 회사들만
  구조적으로 커버 불가 — report 모듈 리더가 저장 상한 확인 필요.
