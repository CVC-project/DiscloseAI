...
GLOSSARY: Dict[str, DisclosureGlossaryEntry] = {
    ...
    "subsidiary": DisclosureGlossaryEntry(
        label="종속기업",
        description="다른 회사 지분을 50% 초과 보유해 사실상 지배하고 있는 자회사 관계.",
        category="equity",
        how="K-IFRS 기준 — 의결권 50% 초과 또는 실질지배 입증 시 종속기업. 자회사 매출·이익·자산을 모회사 재무제표에 통째로 합산(=연결재무제표).",
        benchmark="연결 대상이 늘면 매출·이익이 갑자기 커 보일 수 있으니, 별도재무제표(=모회사 단독)와 같이 보는 것이 정확.",
        intuition="다른 가게를 절반 넘게 사서 '내 가게'로 둔 셈 — 그 가게 매출이 내 장부에 통째로 합쳐짐.",
    ),
    "associate_company": DisclosureGlossaryEntry(
        label="관계기업",
        description="다른 회사 지분을 20~50% 보유해 '중대한 영향력'은 있지만 지배는 못 하는 관계.",
        category="equity",
        how="지분법(Equity method) 적용 — 그 회사의 손익 중 우리 지분 비율만큼만 우리 손익에 반영. 매출·자산은 통째 합산하지 않음.",
        benchmark="관계기업의 적자가 크면 우리 이익도 그 비율만큼 깎임. 관계기업 손상은 갑작스러운 손실로 잡힐 수 있음.",
        intuition="다른 가게 지분을 한 30~40% 가진 상태 — 통제는 안 되지만 그 가게가 번 돈의 비율만큼은 내 장부에 잡힘.",
    ),
    "significant_interest": DisclosureGlossaryEntry(
        label="유의적 투자",
        description="다른 회사 지분을 5~20% 보유. 단순 투자보다 한 단계 영향력 있는 지위.",
        category="equity",
        how="회계상으로는 보통 '단순 투자'(공정가치 측정)와 비슷하게 처리되지만, 사업보고서·관계회사 주석에는 별도 공시.",
        benchmark="20% 미만이라 지분법은 적용 안 함. 다만 5% 이상은 [[mass-holding-report]](5% 룰) 신고 의무.",
        intuition="다른 가게에 작은 지분을 들고 있어 '같은 식구는 아니지만 친한 사이' 정도 — 우리 손익에 직접 섞이진 않음.",
    ),
    # ─────────────────────────────────────────────────────────────
    # 회계 개념 (accounting) — 기본 재무 항목 추가
    # ─────────────────────────────────────────────────────────────
    "revenue": DisclosureGlossaryEntry(
        label="매출(매출액)",
        description="회사가 제품·서비스를 팔아서 1년간 거둬들인 총 금액. 손익계산서 가장 위에 오는 숫자.",
        category="accounting",
        how="할인·반품·세금(부가가치세 등)을 뺀 순매출이 기준. 매출액 = 판매가격 × 판매수량의 합.",
        benchmark="매출이 매년 늘면 성장기업, 정체·감소면 사업 둔화 신호. 다만 매출이 늘어도 이익이 안 늘면 '판매는 되는데 남는 게 없는' 상태.",
        intuition="가게가 1년 동안 손님에게서 받은 돈의 총합 — '얼마나 많이 파나'의 가장 기본 지표.",
    ),
    "operating_income": DisclosureGlossaryEntry(
        label="영업이익",
        description="회사가 본업으로 번 이익. 매출에서 원가·인건비·판관비를 모두 뺀 값.",
        category="accounting",
        how="영업이익 = 매출 − 매출원가 − 판매비 및 관리비. 이자비용·세금은 빼기 전 단계.",
        benchmark="영업이익이 (+) 흑자여야 본업이 굴러가는 회사. 적자가 3~4년 연속이면 사업 모델 점검 필요.",
        intuition="'본업으로 얼마 남는지' — 부업 수익이나 이자수익은 빼고 본 회사 진짜 실력.",
    ),
    "net_income": DisclosureGlossaryEntry(
        label="순이익(당기순이익)",
        description="회사가 1년 동안 모든 비용·세금·일회성 손익까지 다 빼고 최종으로 남긴 이익.",
        category="accounting",
        how="순이익 = 영업이익 ± 영업외손익(이자수익·이자비용·환차익 등) − 법인세. EPS·ROE 계산의 출발점.",
        benchmark="영업이익은 흑자인데 순이익은 적자라면 이자·일회성 손실이 큰 회사. 반대 패턴은 부업·환차익 의존.",
        intuition="가게의 1년 최종 성적표 — 모든 빼고 더한 끝에 손에 남은 돈.",
    ),
    "debt_ratio": DisclosureGlossaryEntry(
        label="부채비율",
        description="회사가 자기 자본 대비 얼마나 많은 빚을 끼고 있는지 보는 지표.",
        category="accounting",
        how="부채비율 = 부채총계 ÷ 자본총계 × 100. 부채가 자본의 2배면 200%.",
        benchmark="제조업 기준 100% 이하 안정, 200% 주의, 400%+ 위험. 단 업종에 따라 다름(금융업은 본질적으로 높고, IT는 낮은 편).",
        intuition="'내 돈 1만원에 빌린 돈 얼마가 같이 굴러가나' — 200%면 내 돈 1만원에 빌린 돈 2만원.",
    ),
    # ─────────────────────────────────────────────────────────────
