...
GLOSSARY: Dict[str, GlossaryEntry] = {
    ...
    "M1": GlossaryEntry(
        label="M1 현금이익률",
        description="본업에서 번 영업이익이 진짜 영업현금으로 돌아왔는가를 측정.",
        how="R = 3년 누적 영업활동현금흐름(OCF) ÷ 3년 누적 영업이익. R=1.0이면 75점, R=2.0이면 100점, R≤0이면 0점 (선형 보간).",
        benchmark="R≥2.0 → 100점 (영업이익의 2배 현금) / R=1.0 → 75점 (이익=현금) / R=0.5 → 37.5점 / R≤0 → 0점",
        intuition="장부 이익이 실제 현금으로 들어왔는지. 영업이익만큼 현금이 들어왔으면 75점, 그보다 더 들어왔으면 100점까지 가산.",
    ),
    "M2": GlossaryEntry(
        label="M2 매출 회수 건전성",
        description="매출증가율과 외상(매출채권+계약자산)증가율의 차이로 매출 신뢰도 평가.",
        how="(미수채권증가율) − (매출증가율) = D. D ≤ -10%p → 100점, 0%p → 80점, +10%p → 50점, +30%p+ → 0점. 건설·조선은 매출채권에 계약자산(미청구공사) 합산.",
        benchmark="-10%p↓ 매우 양호 / 0%p 정상 / +10%p 주의 / +30%p↑ 강한 의심",
        intuition="매출은 10% 늘었는데 외상은 30% 늘면 의심이다. 금융업은 매출채권 개념 부적합으로 제외.",
    ),
    "M3": GlossaryEntry(
        label="M3 부채 건전성",
        description="부채총계 ÷ 자본총계. 빚이 자기 돈의 몇 배인지를 본다.",
        how="업종별 임계값 [T1·T2·T3·T4] 4구간 선형 보간 — D/E=0 → 100점, T1 → 80점, T2 → 60점, T3 → 35점, T4 → 0점. 업종(IT 30~250%, 제조 50~400%, 건설 100~600%, 은행 800~2500%)마다 임계값이 다름.",
        benchmark="T1(양호선) 이내 → 80점 이상. 자본잠식(자본 ≤ 0)은 0점 + 상장폐지 사유.",
        intuition="동일 업종 안에서 빚 부담이 적은 회사를 가린다. 건설·항공이 200%여도 정상이고, 소프트웨어가 100%면 이례적.",
    ),
    "M4": GlossaryEntry(
        label="M4 본업 안정성",
        description="3년 영업이익률의 평균(꾸준히 돈을 내는가) + 변동폭(들쭉날쭉한가).",
        how="평균점수 × 0.6 + 변동폭점수 × 0.4. 평균·변동폭 각각 업종별 임계값. 사이클 업종(반도체·조선·정유)은 변동폭 임계값이 5배 완화.",
        benchmark="해당 업종 우수선(80점) 이상이 우량. 평균이 적자거나 변동폭이 임계 초과면 0점.",
        intuition="고마진 업종(SW·바이오)은 OM 25%가 80점, 저마진 업종(유통)은 OM 4%가 80점. 본질 차이를 임계값에 내재화.",
    ),
    "M5": GlossaryEntry(
        label="M5 자본 성장성",
        description="2년간 자기자본의 연평균 성장률(CAGR). 주주의 몫이 커지는 속도.",
        how="CAGR = (자본총계[t] ÷ 자본총계[t-2])^(1/2) − 1. 5단계 선형 보간: -10%/yr → 0점, 0%/yr → 50점, +10%/yr → 80점, +20%/yr → 95점, +30%/yr → 100점.",
        benchmark="+10%/yr → 80점 (양호) / 0%/yr → 50점 (정체) / 마이너스는 자본 감소(자사주 소각·배당 환원 포함)",
        intuition="회사가 벌어서 안에 쌓이는 속도. 단, 자사주 매입·소각이 활발하면 CAGR이 부당하게 낮을 수 있음.",
    ),
}
