...
class TestRequiredKeys:
    ...
    def test_all_required_keys_present(self, quiz):
        ...
        assert (
            not missing_keys
        ), f"Missing keys in quiz {quiz.get('id')}: {missing_keys}"
...
class TestDataTypes:
    ...
    def test_company_is_string(self, quiz):
        ...
        assert isinstance(
            quiz["company"], str
        ), f"company is not str: {type(quiz['company'])}"
    ...
    def test_ticker_is_string(self, quiz):
        ...
        assert isinstance(
            quiz["ticker"], str
        ), f"ticker is not str: {type(quiz['ticker'])}"
    ...
    def test_kospi_change_pct_is_numeric(self, quiz):
        ...
        assert isinstance(
            quiz["kospi_change_pct"], (int, float)
...
class TestAnswerValidation:
    ...
    def test_answer_is_valid_category(self, quiz):
        ...
        assert (
            quiz["answer"] in self.VALID_ANSWERS
...
class TestTickerFormat:
    ...
    def test_ticker_is_6_digit_number(self, quiz):
        """ticker는 6자리 숫자이다"""
        ticker = quiz["ticker"]
        assert len(ticker) == 6, f"Quiz {quiz['id']}: ticker '{ticker}' is not 6 chars"
        assert (
            ticker.isdigit()
...
class TestEdgeCases:
    ...
    def test_no_empty_strings(self):
        ...
        for idx, quiz in enumerate(QUIZ_LIST):
            for field in string_fields:
                assert quiz[
                    field
                ].strip(), f"Quiz {quiz['id']}: '{field}' is empty or whitespace"
    ...
    def test_context_and_explanation_have_content(self):
        ...
        for quiz in QUIZ_LIST:
            assert len(quiz["context"]) > 20, f"Quiz {quiz['id']}: context too short"
            assert (
                len(quiz["explanation"]) > 20
