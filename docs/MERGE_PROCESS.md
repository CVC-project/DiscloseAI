...
```
[팀원 로컬]                    [원격 GitHub]              [리더]
 feat/financial                  origin/feat/financial
   │
   ├ 작업 + 커밋                     
   │                                                      
   ├ git push ───────────────→ origin/feat/financial 갱신
   │
   │                            GitHub 웹에서
...
git pull                          # 본인 브랜치 원격 변경 받기 (집 PC↔회사 PC 동기화 등)
```
...
2. **base: `dev`**, **compare: `feat/본인담당`**
3. 제목 + 설명 작성 → "Create pull request"

---

## 6. 리더의 PR 처리 (feat/* → dev, 매주 월요일)
...
6. feat/\* 브랜치는 **삭제하지 않음** (계속 재사용)

## 7. 리더 본인 작업 (relation + 리드)

- `feat/relation`에서 작업한 부분도 본인이 PR 생성 → self-merge 가능
...
> 매일 본인 feat를 `pull`, 주기적으로 `dev`를 `merge`, `main`은 리더만.
