...
- **M2 — Vercel 1차 연결 (api/ 최초 코드 = 얇은 프록시) — 배포 후 실패 1건 수정 완료(2026-07-22)**: `integration/api/chat.py`·`integration/api/health.py`(stdlib만 사용 — 의존성 설치 불필요) = DartChatbot 프록시. 오리진 주소는 **Vercel 환경변수 `DART_CHAT_ORIGIN`에만** 둔다(리포·프론트에 서버 주소·터널 URL 미기재 — 보안 규칙). `chat.py`가 C5 면책 문구 부착 + 콜드스타트 단위 최소 rate limit(IP당 분당 20회, M4에서 외부 저장소 기반으로 대체 예정) 담당. 프론트는 M0 덕에 무수정(same-origin `/api/chat`).
  - **Root Directory 실사(2026-07-22)**: main 머지(PR #59) 후 Vercel 대시보드에서 Root Directory=`integration` 선택이 실제로 가능해짐(main에 `integration/` 반영됨). 이렇게 배포한 결과 `/v2/index.html`은 200이나 `/api/health`·`/api/chat`은 **404 실측** — Root Directory가 `integration`이면 그 바깥(repo 루트)의 `api/`는 Vercel이 인식하지 못함. 애초 repo 루트 `api/` + rewrite용 `vercel.json` 조합은 이 조합에서 성립하지 않는 것으로 확인.
  - **수정**: `api/chat.py`·`api/health.py`를 **`integration/api/`로 이동**(Root Directory 안에 위치 — Vercel 기본 관례와 정합), 더 이상 불필요해진 루트 `vercel.json`(rewrite 방식)은 제거. Root Directory=`integration` 유지, Framework Preset=Other.
  - Vercel 프로젝트 연결 자체는 §7.1 절차. `DART_CHAT_ORIGIN` 환경변수에 현재 ngrok 값(M1에서 고정 도메인으로 교체 예정) 등록 필요.
