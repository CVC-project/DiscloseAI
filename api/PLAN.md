...
- **M2 — Vercel 1차 연결 (api/ 최초 코드 = 얇은 프록시) — 코드 작성 완료(2026-07-22), 배포 검증 대기**: `api/chat.py`·`api/health.py` (repo 루트, stdlib만 사용 — 의존성 설치 불필요) = DartChatbot 프록시. 오리진 주소는 **Vercel 환경변수 `DART_CHAT_ORIGIN`에만** 둔다(리포·프론트에 서버 주소·터널 URL 미기재 — 보안 규칙). `api/chat.py`가 C5 면책 문구 부착 + 콜드스타트 단위 최소 rate limit(IP당 분당 20회, M4에서 외부 저장소 기반으로 대체 예정) 담당. 프론트는 M0 덕에 무수정(same-origin `/api/chat`).
  - **Root Directory 이슈**: Vercel 프로젝트 설정의 "Root Directory=integration" 방식은 main에 아직 `integration/`이 없어 대시보드에서 선택 불가(2026-07-22 실측 — main은 dev보다 187커밋 뒤처짐, 마지막 병합 2026-05-06). 대신 **repo 루트에 `vercel.json` rewrite**로 우회: `api/`는 루트에 그대로 둬 Vercel 기본 관례로 함수 인식, 그 외 경로는 `/integration/$1`로 재작성 — Root Directory 설정 자체가 불필요해짐. **미검증** — 실제 프리뷰 배포에서 정적 자산 상대경로·API 라우팅이 의도대로 동작하는지 확인 필요(그라운딩 필요 항목).
