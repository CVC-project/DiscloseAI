...
- **길 A — 임베딩도 API로 (권장·최단, GPU 완전 불필요):** 문서·질문 둘 다 호스팅 임베딩 API(Gemini/OpenAI/Voyage 등). RAG에 GPU가 **한 번도** 안 들어감. 비용은 문서 인덱싱이 1회성 유계(48사 ≈ 몇 센트, 전 상장사 ≈ 대략 $15~40) + 질문당 거의 0. GPU는 밸류체인 QLoRA(진짜 GPU 필요) 전용으로만 남음. 사업보고서는 공개 데이터라 API 전송에 프라이버시 문제 없음.
- **길 B — 문서 인덱싱만 GPU (§3.1 비용원칙 유지):** 수백만 청크를 자체 GPU로 무료 임베딩. 질문은 **같은 모델을 api 서버 CPU에 얹어** 임베딩(위 오해방지 박스 — 같은 모델이지 같은 GPU가 아님). 이때 모델이 **작을수록 CPU 질의가 빨라** GPU를 아예 안 불러도 되므로 소형 모델(gte-small·bge-small·e5-small급, Supabase Edge에 gte-small 내장) 권장. ← "소형" 은 정확성 조건이 아니라 **CPU 질의 지연 최적화**. 초대형 모델만 아니면 CPU로 충분.

**추천 = 길 A.** 목표가 "단순 + GPU 비의존"이면 길 A가 정답이고, 그럼 모델 크기 제약을 신경 쓸 필요가 없다. §3.1의 "임베딩=GPU"는 Graphiti(사용자 행동마다 임베딩 → 무한 고빈도)의 비용 폭주를 막으려던 것 — **유계인 문서 코퍼스 인덱싱엔 API로 충분**. 자체 도메인 파인튜닝 임베딩을 만들고 싶어질 때만 길 B로.

- 어느 길이든 **인덱싱 모델 = 질의 모델** 철칙은 지킨다(모델명·차원·버전을 C2 테이블 메타에 태깅).
- **Vercel 함수 제약(길 B 선택 시)**: torch 등 무거운 ML은 서버리스에 안 맞음 → 질의 임베딩은 Supabase Edge로 빼고 함수는 경량 유지. 길 A는 API 호출뿐이라 이 문제 자체가 없음.
- 실사(§4.1)에서 **인덱스 0건** 확인 → 백지라 지금 자유롭게 선택. 팀원이 이미 임베딩했다면 어떤 모델인지 C2에서 확인 후 정합/재인덱싱.
