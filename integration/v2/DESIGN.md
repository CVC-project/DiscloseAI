...
- **밸류체인 레이어 (U3 활성, 2026-07-28 — UX-013·014·015)**: 상/하 = **흐름(type)에서 파생**(supply=공급처=위 / customer=고객사=아래 — up/down 배열은 미러 기록이라 무시), (상대,type) 병합=최신 as_of. 문법(U-D14): 색=**은백 `#e8f1ff` 단일**(3축 미충돌) + 선=신뢰등급 + **오픈 셰브런**=흐름 위→아래. 노드색=섹터색 유지, **노드 크기=신뢰등급**(T1 크게/T2 작게+빈 테두리). 가로축 없음. 토글=데이터 보유 기업만 활성(현 T1 509사), 범례는 FLOW TYPOLOGY로 통째 교체(혼합 금지).
  - **레일 토폴로지(UX-015~017)** — 방사형이 아니다. 바깥부터 `기업(세로 스택 1행 1사) → 산업 라벨(N사·금액) → 레일 → 트렁크 → 앵커`. 기업마다 앵커까지 선을 뽑지 않는다. 캡=**산업군 5 · 그룹당 4사**(그룹 순서=최상위 멤버 랭킹), 초과는 "외 n개 산업" 묶음(팝오버는 산업별 섹션). **색(UX-017)**: 골격(트렁크+레일)=**앵커의 섹터색** / 스파인=**그 산업의 섹터색** — 셰브런·트렁크 라벨도 앵커색. 레일 범위=스파인 x min~max 클램프, 세그먼트 폭 상한 0.42+중앙 정렬(소수 그룹 좌편향 방지), 모든 라벨은 measureText 축약(세그먼트 침범 금지).
