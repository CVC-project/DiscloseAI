...
- **밸류체인 레이어 (U3 활성, 2026-07-28 — UX-013·UX-014)**: 상/하 = **흐름(type)에서 파생**(supply=공급처=위 / customer=고객사=아래 — up/down 배열은 미러 기록이라 무시), (상대,type) 병합=최신 as_of. 문법(U-D14): 색=**은백 `#e8f1ff` 단일**(3축 미충돌) + 선=신뢰등급(T1 실선/T2 옅은/T3 점선) + **오픈 셰브런**=흐름 위→아래. 노드색=섹터색 유지(엣지 문법만 교체). 가로축 없음. 토글=데이터 보유 기업만 활성(현 T1 509사), 범례는 FLOW TYPOLOGY로 통째 교체(혼합 금지). 라벨=`공급처|고객사 · 금액 · 연도`.
