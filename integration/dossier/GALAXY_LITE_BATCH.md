...
- [x] Phase 0 배선 + 골든 20본 무회귀 (2026-08-11)
- [x] L1 대상 전수 lite JSON + 게이트 PASS + 렌더 스모크 — **22사 완주, 카카오 1사 원천 결함 보류(FN-025)**
- [x] `galaxy_lite_index.json` 반영 — v2 셸에서 해당 기업 탭② 활성 확인
- [ ] 표본 3사 리더 검수 (문체·서사·딥링크) → 통과 시 Wave L2 매핑 결정으로

## 7. 진행 기록

### 세션 1 (2026-08-11, `feat/galaxy-lite-wave1`)

**Phase 0 완료** — `build_galaxy_lite_index.py` 신설 · v2 탭② 골든/lite src 분기(`tabEnabled`/`tabSrc`) · 캐시버스트 m1q→m1r.
골든 무회귀: `galaxy_index.json` 20본 불변(diff 0) · 매니페스트 교집합 0 · 005930·015760 탭② = `galaxy.html` 유지 · 견본 009150 재빌드 결과 **바이트 동일**.
`build_galaxy_index.py` 잠복 결함 1건 선제 수정(글롭이 lite 산출물을 가짜 골든으로 삼킴 — FN-022).

**완주 3사** (모두 게이트 PASS, 렌더 스모크 카드 9장·bridge 7·딥링크 9·콘솔 신규 에러 0)

| 티커 | 회사 | 표준 | 8상 | fact | 눈여겨볼 곳 서사 |
|---|---|---|---|---|---|
| 042700 | 한미반도체 | 삼성전자 | 정리형 | 30 | 지분 처분(+ICF) → 주주환원(−FIN) → 매출채권 회수(+OCF) → 재고 평가손실 |
| 000270 | 기아 | 현대자동차 | 자가발전형 | 28 | 원가 상승 → 재고 증가 → 판매보증충당부채 → 배당 확대 |
| 006400 | 삼성SDI | LG화학 | 확장투자형 | 31 | 유상증자 → 미가동 설비 → 품질보상충당부채 → 부문 손익 |

> 3사 모두 notes 4장 = 8상 세 부호(OCF·ICF·FIN)를 각각 설명 + 이익의 구조 1장 구성으로 수렴했다.
> 이 배치가 §6.5 신호 규칙과 §6.6 서사(조달→사용→마찰→구조)를 동시에 만족시켜, 다음 세션도 이 틀로 시작할 것.

**빌더 교정 3종** (FN-022 — 22사에 그대로 반복될 결함이라 선행 수정): 표준 재무활동 부호·표준 패턴을 **읽어서** 서술 / 적자 기업 문구 분기 / cp949 stdout 고정(게이트 PASS가 FAIL로 보이던 문제).

**세션 1 후반 — 추가 완주 4사** (전부 게이트 PASS + 회귀 게이트 R1~R15 PASS)

| 티커 | 회사 | 표준 | 8상 | 눈여겨볼 곳 서사 |
|---|---|---|---|---|
| 012330 | 현대모비스 | 현대자동차 | 자가발전형 | 부문 이익 편중(A/S) → 지분법이익 39.3% → 보증 약속 → 주주환원 |
| 373220 | LG에너지솔루션 | LG화학 | 확장투자형 | 차입 +46% → 설비 완공 → 유럽 −30.6% → 영업이익<세액공제 |
| 096770 | SK이노베이션 | 고려아연 | 확장투자형 | 만기 당겨짐 → 설비 손상 5조 → 이자 1.41조 → 부문 손익 |
| 207940 | 삼성바이오로직스 | 셀트리온 | 자가발전형 | 인적분할 → 작년 숫자 재작성 → 공장 완공 → 계약부채 |

**세션 1 최종 — Wave L1 완주 (2026-08-11)**

**22사 완주** (견본 삼성전기 포함, 전부 게이트 PASS + 회귀 게이트 R1~R15 PASS, 카드 5+4장) — 매니페스트 22사:
009150 · 042700 · 000270 · 006400 · 012330 · 373220 · 096770 · 207940 · 329180 · 009540 · 010140 · 042660 · 064350 · 079550 · 272210 · 267260 · 000150 · 034020 · 003670 · 028260 · 010120 · 298040
**보류 1사**: 카카오 035720 — 원천 결함(§8, FN-025). L1 대상 23사 중 22사 완주.

**클러스터 시그니처 확립** — 다음 Wave에서 그대로 재사용:
| 유형 | 서사 |
|---|---|
| 수주산업(조선·방산·중공업) | 계약부채(미리 받은 돈) → 설비 → 계약자산(아직 청구 전) → 부문 손익 |
| 제조·소재 | 조달(차입·증자) → 미가동 설비 → 재고·매출채권 → 부문 손익 |
| 지주·복합 | 자본 변동 원인 → 부문 이익 편중 → 계약/운전자본 → 차입 관리 |

**개별 특이점(리더 검수 시 볼 만한 곳)**: LG에너지솔루션 영업이익 1.35조 < 세액공제 1.65조 · SK이노베이션 설비 손상 5.00조 · 삼성바이오로직스 인적분할로 작년 매출 재작성 · 삼성중공업 기타충당부채 440억→9,032억 · 현대모비스 순이익의 39.3%가 지분법 · 삼성물산 자본 증가는 평가이익.
...
| 티커 | 회사 | 사유 |
|---|---|---|
| 035720 | 카카오 | **원인 정정(FN-025)**: '기준 혼재'가 아니라 **계정 오매칭**. `fs_account`엔 FY23 영업수익 7.56조·이자수익 0.19조가 둘 다 있는데 firm JSON이 **이자수익**을 매출로 가져갔다. 뿌리는 `modules/financial/collector.py`의 `"이자수익": "revenue"` 매핑(금융사용인데 조건이 없음). **비금융·비지주 269사 동일 증상**(골든 한화에어로 012450 FY25 매출 0.22조 포함 — 단 골든 은하수는 report 자체 series라 무영향, 영향 표면은 EQS 탭). financial 담당자 수정 후 재개 — S1 selfcheck가 자동 판정한다. |
